...
export const SourceSchema = z.discriminatedUnion('kind', [
  ...
  z.object({
    kind: z.literal('search'), value: z.string().trim().min(1).max(500),
    maxImages: z.number().int().min(1).max(100).default(100),
    strategy: z.enum(['balanced', 'newest', 'oldest']).default('newest')
  }),
